...
// workqueue and outbox writers. Implementations must not begin nested
...
func New(beginner Beginner, options pgx.TxOptions) *PGXUnitOfWork {
	if beginner == nil {
		panic("platform transaction beginner is nil")
	}
...
func newWithBegin(begin beginFunc, options pgx.TxOptions) *PGXUnitOfWork {
	if begin == nil {
		panic("platform transaction begin function is nil")
	}
...
func (u *PGXUnitOfWork) WithinTransaction(
	...
	if ctx == nil {
		return errors.New("platform transaction context is nil")
	}
	if fn == nil {
		return errors.New("platform transaction callback is nil")
	}
	...
	defer func() {
		...
		if rollbackErr != nil {
			err = errors.Join(err, fmt.Errorf(
				"rollback platform transaction: %w",
				rollbackErr,
			))
		}
	}()
	...
	if err := fn(ctx, tx); err != nil {
		return fmt.Errorf("run platform transaction: %w", err)
	}
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("platform transaction context: %w", err)
	}
