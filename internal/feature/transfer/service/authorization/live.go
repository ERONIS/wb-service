...
func (actor LiveTrustedActor) digest() Digest {
	builder := canonicalhash.NewSHA256("transfer-live-actor:v1")
	builder.Int64(actor.TelegramUserID)
...
type AuthorizationPlanSummary struct {
	...
	PlanRevision  int64
	// LatestAuthorizationID is the persisted renewal generation. Automatic
	// authorization keys include it so retries for the same generation replay,
	// while a plan whose previous authorization expired receives a fresh key.
	LatestAuthorizationID LiveAuthorizationID
	TargetsCount          int
	CreateActions         int
	AddActions            int
	MediaActions          int
...
func (summary AuthorizationPlanSummary) Validate() error {
	if summary.TransferID <= 0 || summary.PlanID <= 0 ||
		...
		summary.PlanRevision < 0 || summary.LatestAuthorizationID < 0 ||
		summary.TargetsCount <= 0 || summary.CreateActions < 0 ||
...
func (command RequestLiveCommand) digest() Digest {
	builder := canonicalhash.NewSHA256("transfer-request-live:v1")
	builder.Int64(int64(command.TransferID))
	builder.Bytes(command.PlanDigest[:])
	return Digest(builder.Sum())
}
...
func (command ApproveLiveCommand) digest() Digest {
	builder := canonicalhash.NewSHA256("transfer-approve-live:v1")
	builder.Int64(int64(command.TransferID))
	builder.Int64(int64(command.AuthorizationID))
	builder.Int64(command.ExpectedRevision)
	builder.Bytes(command.ExpectedPlanDigest[:])
	return Digest(builder.Sum())
}
...
func (command ChangeLiveAuthorizationStateCommand) digest(state LiveAuthorizationState) Digest {
	builder := canonicalhash.NewSHA256("transfer-change-live-state:v1")
	builder.String(string(state))
	builder.Int64(int64(command.TransferID))
	builder.Int64(int64(command.AuthorizationID))
	builder.Int64(command.ExpectedRevision)
	builder.Bytes(command.ExpectedPlanDigest[:])
	builder.String(command.SafeReasonCode)
	return Digest(builder.Sum())
}
...
func (command RevokeLiveCommand) digest() Digest {
	builder := canonicalhash.NewSHA256("transfer-revoke-live:v1")
	builder.Int64(int64(command.TransferID))
	builder.Int64(int64(command.AuthorizationID))
	builder.Int64(command.ExpectedRevision)
	builder.String(command.SafeReasonCode)
...
func (check LiveAuthorizationCheck) validate() error {
	if check.TransferID <= 0 || check.ActionID <= 0 || check.AuthorizationID <= 0 ||
		...
		check.CabinetID == "" || check.SellerKey == (SellerKey{}) ||
