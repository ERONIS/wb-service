...
func (c FinalizeCommand) digest() Digest {
	hasher := sha256.New()
	canonicalWriteString(hasher, "cardimport-finalize-command:v1")
	canonicalWriteInt64(hasher, int64(c.SessionID))
	canonicalWriteInt64(hasher, c.ExpectedRevision)
	canonicalWriteString(hasher, c.IdempotencyKey)
